...
let rec fv = function (* 式に出現する（自由な）変数 (caml2html: knormal_fv) *)
  ...
  | LetRec(fundefs, e2) ->
      let (xs, zs) =
	List.fold_left
	  (fun (xs, zs) { name = (x, t); args = yts; body = e1 } ->
	    (S.add x xs,
	     S.union zs (S.diff (fv e1) (S.of_list (List.map fst yts)))))
	  (S.empty, S.empty)
	  fundefs in
...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Bool(b) -> Int(if b then 1 else 0), Type.Bool (* 論理値true, falseを整数1, 0に変換 (caml2html: knormal_bool) *)
  ...
  | Syntax.LetRec(fundefs, e2) ->
	...
	M.add_list
      ...
      LetRec(List.map
	       (fun { Syntax.name = xt; Syntax.args = yts; Syntax.body = e1 } ->
