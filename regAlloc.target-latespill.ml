...
and g'_and_restore dest cont regenv exp = (* 使用される変数をスタックからレジスタへRestore (caml2html: regalloc_unspill) *)
  ...
  with NoReg(x, t) ->
    ...
    let all =
      ...
      | Type.Float -> allfregs
      | _ -> allregs in
    (match alloc dest cont' regenv all x with
    | Spill(y) ->
	let r = M.find y regenv in
	let (e1', regenv1) = g'_and_restore dest cont (add x r (M.remove y regenv)) exp in
	(seq(Save(r, y), Let((r, t), Restore(x), e1')), regenv1)
    | Alloc(r) ->
