...
let rec g env = function (* 命令列の13bit即値最適化 (caml2html: simm13_g) *)
  ...
  | Let(x, Set(i), e) when (-4096 <= i) && (i < 4096) ->
      Format.eprintf "found simm13 %s = %d@." x i;
      let e' = g (M.add x i env) e in
      if List.mem x (fv e') then Let(x, Set(i), e') else
      (Format.eprintf "erased redundant Set to %s@." x;
       e')
  | Let(x, SLL(y, C(i)), e) when M.mem y env -> (* for array access *)
      Format.eprintf "erased redundant SLL on %s@." x;
      g env (Let(x, Set((M.find y env) lsl i), e))
  | Seq(exp, e) -> Seq(g' env exp, g env e)
  | Let(x, exp, e) -> Let(x, g' env exp, g env e)
  | FLetD(x, exp, e) -> FLetD(x, g' env exp, g env e)
